...
{
    private static readonly Regex SerialSuffixPattern = new("^[0-9]{1,2}$", RegexOptions.Compiled);
    ...
    private static async Task<int> Main(string[] args)
    {
        ...
        Console.WriteLine($"Vyhľadávam SmartLPR kamery na https://192.168.1.2XX:8080/api/v2/status (timeout {options.TimeoutSeconds} s)...");
    ...
    private static async Task<QueryOutcome> QueryEndpointAsync(HttpClient httpClient, Uri uri, CancellationToken cancellationToken)
    {
        try
        {
            ...
            try
            {
                ...
                return new QueryOutcome(true, document.RootElement.Clone(), message, response.StatusCode, rawContent);
            }
    ...
    private static void PrintVersionSummary(string ipSuffix, JsonElement payload)
    {
        ...
        static void PrintStringProperty(string suffix, JsonElement obj, string propertyName, string label)
        {
        ...
        if (payload.TryGetProperty("modules", out var modules) && modules.ValueKind is JsonValueKind.Array)
        {
...
");
    ...
    {
        public List<string> SerialSuffixes { get; } = new();
